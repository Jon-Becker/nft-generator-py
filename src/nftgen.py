...
class NFTGenerator:
    ...
    def generate_unique_images(self) -> None:
        ...
        for incomp in self.config["incompatibilities"]:
            if "default" in incomp:
                for layer in trait_files:
                    trait_files[layer][incomp["default"]["value"]] = incomp["default"][
                        "filename"
                    ]
                ...
                if attr != "tokenId":
                    ...
                    if "/" in trait_files[attr][item[attr]]:
                        layers[index] = Image.open(
                            f"{trait_files[attr][item[attr]]}.png"
                        ).convert("RGBA")
                    else:
                        layers[index] = Image.open(
                            f'{self.config["layers"][index]["trait_path"]}/{trait_files[attr][item[attr]]}.png'
                        ).convert("RGBA")
        ...
        all_token_rarity = []
        for layer in self.config["layers"]:
            all_token_rarity.append(
                {layer["name"]: Counter(image[layer["name"]] for image in all_images)}
            )
    ...
    def generate_config(
        self, trait_dir: str, baseURI: str, name: str, description: str
    ) -> None:
        ...
        jsondict = {
            "layers": str(jsonstringarray).replace("'", ""),
            "incompatibilities": [],
            "baseURI": f"{baseURI}",
            "name": name,
